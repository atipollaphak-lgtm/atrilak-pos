...
class CustomerImportStorageService
{
    public function store(
        ...
        $expiresAt = now()->addMinutes((int) config('customer_import.token_ttl_minutes'));
        $payload = [
            'token' => $token,
            'user_id' => $userId,
            'filename' => $filename,
    ...
    {
        return 'customer_import.preview.'.$token;
    }

    private function toData(array $payload): CustomerImportPreviewData
    {
        return new CustomerImportPreviewData(
