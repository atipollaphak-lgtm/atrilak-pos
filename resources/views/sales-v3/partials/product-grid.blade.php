<div id="v3-product-grid" class="pos-v3-grid">
    @foreach ($products as $product)
        ...
        @endphp
        @php($productData = ['id' => $product->id, 'name' => $product->name, 'code' => $product->product_code, 'sku' => $product->sku, 'barcode' => $product->barcode, 'price' => $product->selling_price, 'cost_price' => $product->cost_price, 'rounding_direction' => $product->rounding_direction, 'rounding_unit' => $product->rounding_unit, 'stock_qty' => $product->stock_qty, 'minimum_stock' => $product->minimum_stock, 'unit' => $product->unit, 'category_id' => $product->category_id, 'category_rounding_override' => $product->category?->rounding_override, 'sort_order' => (int) ($product->sort_order ?? 0), 'product_sort_order' => (int) ($product->sort_order ?? 0), 'category_sort_order' => (int) ($product->category?->sort_order ?? 0), 'frequent_order' => $product->frequentProduct?->sort_order, 'image_path' => $product->image_path, 'image_url' => $product->image_url, 'productUnits' => $product->productUnits])
        <button type="button" class="v3-product-card {{ $stockState }}" data-product-id="{{ $product->id }}" data-product='{{ json_encode($productData, JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_TAG | JSON_HEX_AMP) }}' data-name="{{ strtolower($product->name) }}" data-category="{{ $product->category_id }}" data-category-sort-order="{{ (int) ($product->category?->sort_order ?? 0) }}" data-product-sort-order="{{ (int) ($product->sort_order ?? 0) }}" data-frequent-order="{{ $product->frequentProduct?->sort_order ?? '' }}" data-search="{{ strtolower($product->name.' '.$product->product_code.' '.$product->sku.' '.$product->barcode) }}">
