(function () {
    ...
    function showSuccess(data) { context.lastSaleId = data.sale_id; $('#final-payment-close')?.classList.add('d-none'); $('#final-payment-status')?.classList.remove('d-none'); $('#final-preview-sale-no').textContent = `เลขที่บิล ${data.sale_no || '-'}`; $('#final-confirm-payment')?.classList.add('d-none'); $('#final-edit-items')?.classList.add('d-none'); $('#final-document-panel')?.classList.remove('d-none'); $('#payment-confirmation-modal')?.classList.add('has-documents'); $('#final-print-documents').disabled = false; $('#final-finish-payment').disabled = false; window.jQuery('#payment-confirmation-modal').modal('show'); }
    function ensurePaymentMethodSummary() { const confirm = $('#final-confirm-payment'); if (!confirm || $('#final-payment-method-summary')) return; const summary = document.createElement('div'); summary.id = 'final-payment-method-summary'; summary.className = 'final-payment-method-summary'; const label = document.createElement('strong'); label.textContent = 'วิธีชำระเงิน: เงินสด'; const change = document.createElement('button'); change.type = 'button'; change.className = 'btn btn-link btn-sm'; change.textContent = 'เปลี่ยนวิธีชำระเงิน'; change.addEventListener('click', () => { window.jQuery('#payment-confirmation-modal').modal('hide'); context.payment.open(); }); summary.append(label, change); confirm.parentElement?.insertBefore(summary, confirm); }
